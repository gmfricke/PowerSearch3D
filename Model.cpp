...
void Model::generate()
{
    ...
    filepath << save_path << "/" << getSearchSpace()->getSearchTypeName() << "_displacements.csv";
...
void Model::doAnalysis()
{
    ...
    for(unsigned int i = 0; i < input_sets.size(); i++) dataset_labels.push_back("unlabled");

    // An input set can consist of multiple fields.
    int num_total_entries = input_sets.size()*cluster_radii.size();

    int n_repetitions = 10;
    int n_replicas = 100;

    // Hold the repetition efficiencies (mean of replicas)

    vector< vector<double> > results_time_eff_medians(num_total_entries);
    ...
    // Hold the individual replica efficiencies
    vector< vector<double> > results_time_eff_all_data(num_total_entries);
    vector< vector<double> > results_dist_eff_all_data(num_total_entries);
    vector< vector<double> > results_fc_eff_all_data(num_total_entries);
    ...
    for (int m = 0; m < n_repetitions; m++)
    {
        ...
        for (unsigned int i = 0; i < input_sets.size()*cluster_radii.size(); i++)
        {
        ...
        int file_index = 0;
        for (unsigned int h = 0; h < input_sets.size(); h++)
        {
            int setid = h;
            ...
            for (int k = 0; k < queued_input_files->size(); k++)
            {
                        ...
                        || queued_input_files->at(k) == "GammaMCRW")
                {
                    ...
                    model_name = queued_input_files->at(k).toStdString();
                    ...
                    vector<double> speeds = space->getSpeeds();

                    stringstream ss;
                    ss << save_path << "/" << queued_input_files->at(k).toStdString() << "_speeds.csv";

                    writeVectorContentsToCSVFile(speeds, ss.str());
                ...
                //for (int m = 0; m < 3; m++)
                {
                    ...
                    for (int j = 0; j < cluster_radii.size(); j++ )
                    {
                        ...
                        for ( int i = 0; i < n_replicas; i++ )
                        {
                            ...
                            stringstream ind_file_path_ss;
                            ind_file_path_ss << save_path << "/" << model_name << "_track_data.csv";


                            writeIndividualTrackEfficienciesToCSVFile( ind_file_path_ss.str() );

                            double time_eff = num_targets_found/max_time;//time_used;

                            double dist_eff = num_targets_found/max_distance;//distance_used;
                            ...
                            double time_eff_median= 0;
                            double time_eff_lower_quartile = 0;
                            double time_eff_upper_quartile = 0;
                            double time_eff_min = 0;
                            double time_eff_max = 0;
                            double time_eff_mean = 0;
                            double time_eff_interquartile_range = 0;
                            double time_eff_inner_fence = 0;

                            vector<double> time_eff_outliers;

                            double hopkins_mean = 0;
                            double mean_speed_value = 0;
                            //cout << "Number of elements in (*dist_efficiencies[idx]): " << (*dist_efficiencies[idx]).size() << endl;

                            double dist_eff_median = 0;
                            double dist_eff_lower_quartile = 0;
                            double dist_eff_upper_quartile = 0;
                            double dist_eff_min = 0;
                            double dist_eff_max = 0;
                            double dist_eff_mean = 0;
                            double dist_eff_interquartile_range = 0;
                            double dist_eff_inner_fence = 0;
                            vector<double> dist_eff_outliers;

                            double first_contact_median= 0;
                            double first_contact_lower_quartile = 0;
                            double first_contact_upper_quartile = 0;
                            double first_contact_min = 0;
                            double first_contact_max = 0;
                            double first_contact_mean = 0;
                            double first_contact_interquartile_range = 0;
                            double first_contact_inner_fence = 0;

                            vector<double> first_contact_outliers;

                            // Show the results so far in the histogram until the all data vector of results has at least one entry
                            if (results_time_eff_all_data[idx].empty())
                            {

                                time_eff_median = median(*time_efficiencies[idx]);
                                time_eff_lower_quartile = lowerQuartile(*time_efficiencies[idx]);
                                time_eff_upper_quartile = upperQuartile(*time_efficiencies[idx]);
                                time_eff_min = min(*time_efficiencies[idx]);
                                time_eff_max = max(*time_efficiencies[idx]);
                                time_eff_mean = mean(*time_efficiencies[idx]);
                                time_eff_interquartile_range = time_eff_upper_quartile - time_eff_lower_quartile;
                                time_eff_inner_fence = 1.5*time_eff_interquartile_range;

                                time_eff_outliers = calcOutliers(*time_efficiencies[idx], time_eff_inner_fence, time_eff_median);

                                hopkins_mean = mean(*hopkins_values[idx]);
                                mean_speed_value = mean(*mean_speeds[idx]);

                                //cout << "Number of elements in (*dist_efficiencies[idx]): " << (*dist_efficiencies[idx]).size() << endl;

                                dist_eff_median = median(*dist_efficiencies[idx]);
                                dist_eff_lower_quartile = lowerQuartile(*dist_efficiencies[idx]);
                                dist_eff_upper_quartile = upperQuartile(*dist_efficiencies[idx]);
                                dist_eff_min = min(*dist_efficiencies[idx]);
                                dist_eff_max = max(*dist_efficiencies[idx]);
                                dist_eff_mean = mean(*dist_efficiencies[idx]);
                                dist_eff_interquartile_range = dist_eff_upper_quartile - dist_eff_lower_quartile;
                                dist_eff_inner_fence = 1.5*dist_eff_interquartile_range;

                                dist_eff_outliers = calcOutliers(*dist_efficiencies[idx], dist_eff_inner_fence, dist_eff_median);

                                first_contact_median = median(*first_contacts[idx]);
                                first_contact_lower_quartile = lowerQuartile(*first_contacts[idx]);
                                first_contact_upper_quartile = upperQuartile(*first_contacts[idx]);
                                first_contact_min = min(*first_contacts[idx]);
                                first_contact_max = max(*first_contacts[idx]);
                                first_contact_mean = mean(*first_contacts[idx]);
                                first_contact_interquartile_range = first_contact_upper_quartile - first_contact_lower_quartile;
                                first_contact_inner_fence = 1.5*first_contact_interquartile_range;

                                first_contact_outliers = calcOutliers(*first_contacts[idx], first_contact_inner_fence, first_contact_median);
                            }
                            else
                            {
                                time_eff_median = median(results_time_eff_all_data[idx]);
                                time_eff_lower_quartile = lowerQuartile(results_time_eff_all_data[idx]);
                                time_eff_upper_quartile = upperQuartile(results_time_eff_all_data[idx]);
                                time_eff_min = min(results_time_eff_all_data[idx]);
                                time_eff_max = max(results_time_eff_all_data[idx]);
                                time_eff_mean = mean(results_time_eff_all_data[idx]);
                                time_eff_interquartile_range = time_eff_upper_quartile - time_eff_lower_quartile;
                                time_eff_inner_fence = 1.5*time_eff_interquartile_range;

                                time_eff_outliers = calcOutliers(results_time_eff_all_data[idx], time_eff_inner_fence, time_eff_median);
                                ...
                                dist_eff_median = median(results_dist_eff_all_data[idx]);
                                dist_eff_lower_quartile = lowerQuartile(results_dist_eff_all_data[idx]);
                                dist_eff_upper_quartile = upperQuartile(results_dist_eff_all_data[idx]);
                                dist_eff_min = min(results_dist_eff_all_data[idx]);
                                dist_eff_max = max(results_dist_eff_all_data[idx]);
                                dist_eff_mean = mean(results_dist_eff_all_data[idx]);
                                dist_eff_interquartile_range = dist_eff_upper_quartile - dist_eff_lower_quartile;
                                dist_eff_inner_fence = 1.5*dist_eff_interquartile_range;

                                dist_eff_outliers = calcOutliers(*dist_efficiencies[idx], dist_eff_inner_fence, dist_eff_median);

                                first_contact_median = median(results_fc_eff_all_data[idx]);
                                first_contact_lower_quartile = lowerQuartile(results_fc_eff_all_data[idx]);
                                first_contact_upper_quartile = upperQuartile(results_fc_eff_all_data[idx]);
                                first_contact_min = min(results_fc_eff_all_data[idx]);
                                first_contact_max = max(results_fc_eff_all_data[idx]);
                                first_contact_mean = mean(results_fc_eff_all_data[idx]);
                                first_contact_interquartile_range = first_contact_upper_quartile - first_contact_lower_quartile;
                                first_contact_inner_fence = 1.5*first_contact_interquartile_range;

                                first_contact_outliers = calcOutliers(results_fc_eff_all_data[idx], first_contact_inner_fence, first_contact_median);
        ...
        double Slq1t = lowerQuartile(*time_efficiencies[0]);
        double Suq1t = upperQuartile(*time_efficiencies[0]);
        double Smin1t = min(*time_efficiencies[0]);
        double Smax1t = max(*time_efficiencies[0]);
        ...
        double Slq1d = lowerQuartile(*dist_efficiencies[0]);
        double Suq1d = upperQuartile(*dist_efficiencies[0]);
        double Smin1d = min(*dist_efficiencies[0]);
        double Smax1d = max(*dist_efficiencies[0]);
        ...
        results_time_eff_medians[0].push_back(Smed1t);


        cout << "Si_label << ""Observed N samples: " << Sn1t << endl;
        cout << "Observed Median (time): " << Smed1t << endl;
        cout << "Observed Mean (time): " << Sm1t << endl;
        cout << "Observed Stdev (time): " << Sd1t << endl;
        cout << "Observed Min (time): " << Smin1t << endl;
        cout << "Observed Max (time): " << Smax1t << endl;
        cout << "Observed Upper Quartile (time): " << Suq1t << endl;
        cout << "Observed Lower Quartile (time): " << Slq1t << endl;



        cout << "Observed Median (distance): " << Smed1d << endl;
        cout << "Observed Mean (distance): " << Sm1d << endl;
        cout << "Observed Stdev (distance): " << Sd1d << endl;
        cout << "Observed Min (distance): " << Smin1d << endl;
        cout << "Observed Max (distance): " << Smax1d << endl;
        cout << "Observed Upper Quartile (distance): " << Suq1d << endl;
        cout << "Observed Lower Quartile (distance): " << Slq1d << endl;



        results_time_eff_all_data[0].insert(
                    results_time_eff_all_data[0].end(),
                time_efficiencies[0]->begin(),
                time_efficiencies[0]->end()
                );

        results_dist_eff_all_data[0].insert(
                    results_dist_eff_all_data[0].end(),
                dist_efficiencies[0]->begin(),
                dist_efficiencies[0]->end()
                );

        results_fc_eff_all_data[0].insert(
                    results_fc_eff_all_data[0].end(),
                first_contacts[0]->begin(),
                first_contacts[0]->end()
                );
...
*/
        ...
        for (int i = 1; i < dataset_labels.size(); i++)
        {
            for (int j = 0; j < cluster_radii.size(); j++)
            {
                ...
                cout << "Time efficiencies (repetition=" << m << ") :" << endl;
                //cout << "Observed: " << endl;
                //printVectorContents((*time_efficiencies[0]));
                //cout << "dataset=" << Si_label << endl;
                //printVectorContents((*time_efficiencies[index]));

                int n_time_samples = (*time_efficiencies[index]).size();
                int n_dist_samples = (*dist_efficiencies[index]).size();

                cout << "Model::doAnalysis(): N time samples: " << n_time_samples << endl;
                cout << "Model::doAnalysis(): N dist samples: " << n_dist_samples << endl;

                double Smit = mean(*time_efficiencies[index]);
                double Smedit = median(*time_efficiencies[index]);

                double Sdit = unbiasedSampleStandardDeviation(*time_efficiencies[index]);
                double Snit = time_efficiencies[index]->size();
                ...
                double Smid = mean(*dist_efficiencies[index]);
                double Smedid = median(*dist_efficiencies[index]);
                double Sdid = unbiasedSampleStandardDeviation(*dist_efficiencies[index]);
                double Snid = dist_efficiencies[index]->size();
                double Smifc = mean(*first_contacts[index]);
                double Smedifc = median(*first_contacts[index]);
                double Sdifc = unbiasedSampleStandardDeviation(*first_contacts[index]);
                double Snifc = first_contacts[index]->size();

                cout << Si_label << "N samples: " << Snit << endl;
                cout << Si_label << " Median (time): " << Smedit << endl;
                cout << Si_label << " Mean (time): " << Smit << endl;
                cout << Si_label << " Stdev (time): " << Sdit << endl;
                cout << Si_label << " Min (time): " << Smin1t << endl;
                cout << Si_label << " Max (time): " << Smax1t << endl;
                cout << Si_label << " Upper Quartile (time): " << Suq1t << endl;
                cout << Si_label << " Lower Quartile (time): " << Slq1t << endl;

                cout << Si_label << " Median (distance): " << Smedid << endl;
                cout << Si_label << " Mean (distance): " << Smid << endl;
                cout << Si_label << " Stdev (distance): " << Sdid << endl;
                cout << Si_label << " Min (distance): " << Smin1d << endl;
                cout << Si_label << " Max (distance): " << Smax1d << endl;
                cout << Si_label << " Upper Quartile (distance): " << Suq1d << endl;
                cout << Si_label << " Lower Quartile (distance): " << Slq1d << endl;
                ...
                results_time_eff_all_data[index].insert(
                            results_time_eff_all_data[index].end(),
                            time_efficiencies[index]->begin(),
                            time_efficiencies[index]->end()
                            );

                results_dist_eff_all_data[index].insert(
                            results_dist_eff_all_data[index].end(),
                            dist_efficiencies[index]->begin(),
                            dist_efficiencies[index]->end()
                            );

                results_fc_eff_all_data[index].insert(
                            results_fc_eff_all_data[index].end(),
                            first_contacts[index]->begin(),
                            first_contacts[index]->end()
                            );
                ...
                results_time_eff_medians[index].push_back(Smedit);
                ...
                // Keep track of all the samples for the Mann-Whitney test
...
*/
    ...
    stringstream filepath;
    filepath << save_path << "/stats.csv";

    ofstream file_output;
    file_output.open(filepath.str().c_str(), ios::out);

    file_output << "Statistical Results Table (Mean change, Median Change, p-value)" << endl;
    ...
    file_output << "Time Efficiency:" << endl;
    file_output << "Time (cluster radius), ";
    for (int ii = 1; ii < dataset_labels.size(); ii++)
    {
        file_output << dataset_labels[ii].toStdString();
        for (int jj = 0; jj < cluster_radii.size(); jj++)
        {
            file_output << "(" << cluster_radii[jj] << "), ";
        }
    }
    file_output << endl;

    file_output << "% Change Mean, ";
    for (int ii = 1; ii < dataset_labels.size(); ii++)
        for (int jj = 0; jj < cluster_radii.size(); jj++)
        {
            ...
            file_output << time_eff_change_in_mean_conf_interval.first << " < \u03BC  = " << mean(results_time_eff_change_in_mean[index]) << " < " << time_eff_change_in_mean_conf_interval.second << ", ";
        }
    file_output << endl;

    file_output << "% Change Median, ";
    for (int ii = 1; ii < dataset_labels.size(); ii++)
        for (int jj = 0; jj < cluster_radii.size(); jj++)
        {
            int index = ii*cluster_radii.size()+jj;
            ...
            file_output << time_eff_change_in_median_conf_interval.first << " < \u03BC = " << mean(results_time_eff_change_in_median[index]) << " < " << time_eff_change_in_median_conf_interval.second << ", ";
        }
    file_output << endl;

    /*
file_output << "Left p-value, ";
for (int ii = 1; ii < dataset_labels.size(); ii++)
    for (int jj = 0; jj < cluster_radii.size(); jj++)
    {
        int index = ii*cluster_radii.size()+jj;
        pair<double,double> time_left_p_value_conf_interval = unbiasedSampleConfidenceInterval(results_time_eff_left_p_value[index]);
        file_output << time_left_p_value_conf_interval.first << " < \u03BC < " << time_left_p_value_conf_interval.second << ", ";
    }
file_output << endl;

file_output << "Right p-value, ";
for (int ii = 1; ii < dataset_labels.size(); ii++)
    for (int jj = 0; jj < cluster_radii.size(); jj++)
    {
        int index = ii*cluster_radii.size()+jj;
        ...
        file_output << time_right_p_value_conf_interval.first << " < \u03BC < " << time_right_p_value_conf_interval.second << ", ";
    }
file_output << endl;
*/

    file_output << "Two sided p-value (Distribution), ";
    for (int ii = 1; ii < dataset_labels.size(); ii++)
        for (int jj = 0; jj < cluster_radii.size(); jj++)
        {
            int index = ii*cluster_radii.size()+jj;
            //file_output << "Model::doAnalysis(): index " << index << endl;
            ...
            //file_output << time_two_sided_p_value_conf_interval.first << " < \u03BC < " << time_two_sided_p_value_conf_interval.second << ", ";
            file_output << mannwhitney(results_time_eff_all_data[0], results_time_eff_all_data[index]) << " (N="<< results_time_eff_all_data[index].size() <<"), ";

            // file_output << "Number of dists: " << results_time_eff_all_data.size() << endl;

            //stringstream filepath;
            //filepath << "/Users/matthew/Desktop/Observed_time_efficiencies.csv";
            // writeVectorContentsToCSVFile( results_time_eff_all_data[0], filepath.str().c_str() );

            //file_output << dataset_labels[ii].toStdString() << endl;
            //filepath.str(std::string()); // clear contents
            //filepath.clear(); // clear error states
            // filepath << "/Users/matthew/Desktop/" << dataset_labels[ii].toStdString() << "_time_efficiencies.csv";
            // file_output << "Model::doAnalysis: time efficiencies output file path:" << filepath.str() << endl;
            // writeVectorContentsToCSVFile( results_time_eff_all_data[index], filepath.str().c_str() );

        }
    file_output << endl;

    file_output << "Two sided p-value (Change in Median), ";
    for (int ii = 1; ii < dataset_labels.size(); ii++)
        for (int jj = 0; jj < cluster_radii.size(); jj++)
        {
            int index = ii*cluster_radii.size()+jj;
            ...
            //file_output << fc_two_sided_p_value_conf_interval.first << " < \u03BC < " << fc_two_sided_p_value_conf_interval.second;
            file_output << mannwhitney(results_time_eff_medians[0], results_time_eff_medians[index]) << " (N="<< results_time_eff_change_in_median[index].size() <<"), ";
        }

    file_output << endl << endl;

    for (unsigned int h = 1; h < input_sets.size(); h++)
    {
        int setid = h;

        for (int j = 0; j < cluster_radii.size(); j++)
        {
            id = cluster_radii.size()*setid+j;

            QVector<float> sample;

            sample = packageSampleForHistogram(results_time_eff_change_in_median[id], setid, id);

            emit updateFirstContactSample(sample);
        }
    }
    ...
    for ( int i = 0; i < n_searchers; i++ )
    {
        ...
        if (path.size() >= 2)
            for (int j = 0; j < path.size()-1; j++)
            {
                ...
                for(unsigned int k = 0; k < found_targets.size(); k++)
...
void Model::saveSettings()
{
    ...
    config_file << save_path << endl;

    cout << "Saved working path value " << working_path << endl;
    cout << "Saved save path value " << save_path << endl;
...
void Model::readSettings()
{
    ...
    getline(config_file, save_path);

    config_file.close();

    cout << "Model: working_path set to " << working_path << endl;
    cout << "Model: save_path set to " << save_path << endl;
}
...
pair<double, double> Model::populationConfidenceInterval( vector<double> population ) //,  float degree )
{
    ...
    double half_interval = populationStandardDeviation(population)*1.96/sqrt(population.size());
...
{
    limit_by_time = v;
    if (v) cout << "Set to limit by time" << endl;
    else  cout << "Set to limit by distance" << endl;
}

void Model::setDistLimited(bool v)
{
    limit_by_time = !v;
    if (!v) cout << "Set to limit by time" << endl;
    else  cout << "Set to limit by distance" << endl;
...
double Model::mannwhitney(vector<double> values1, vector<double> values2)
{
    double p_twotail = -1;
    double p_lefttail = -1;
    double p_righttail = -1;


    real_1d_array sample1;
    real_1d_array sample2;

    alglib::ae_int_t sample1_size = values1.size();
    alglib::ae_int_t sample2_size = values2.size();

    sample1.setcontent(sample1_size, values1.data());
    sample2.setcontent(sample2_size, values2.data());
...
void Model::writeVectorContentsToCSVFile( vector<double> values, string filepath )
{
    ofstream output_stream;
    output_stream.open(filepath.c_str());

    if (values.empty()) output_stream << "<empty>";
    for(std::vector<double>::iterator j=values.begin(); j!=values.end(); ++j)
        output_stream << *j << ", ";
...
QVector<float> Model::packageSampleForHistogram(vector<double> data, int setid, int id)
{
    QVector<float> sample;

    // Calculate the values for box plot sample
    float data_median = median(data);
    float data_lower_quartile = lowerQuartile(data);
    float data_upper_quartile = upperQuartile(data);
    float data_min = min(data);
    float data_max = max(data);
    float data_mean = mean(data);

    float data_interquartile_range = data_upper_quartile - data_lower_quartile;
    float data_inner_fence = 1.5*data_interquartile_range;

    cout << "Data median: " << data_median << endl;
    cout << "Data inner fence: " << data_inner_fence << endl;

    std::vector<double> data_outliers = calcOutliers(data, data_inner_fence, data_median);

    sample.push_back(id);
    sample.push_back(setid);

    sample.push_back(data_median);
    sample.push_back(data_lower_quartile);
    sample.push_back(data_upper_quartile);
    //time_eff_sample.push_back(time_eff_min);
    //time_eff_sample.push_back(time_eff_max);

        sample.push_back(data_median-data_inner_fence);
        sample.push_back(data_median+data_inner_fence);

        sample.push_back(data_mean);

    //time_eff_sample.insert(time_eff_sample.end(), time_eff_outliers.begin(), time_eff_outliers.end());
    sample.push_back(0); // used to be hopkins mean
    sample.push_back(0); // used to be cluster radius
    sample.push_back(0); // mean value covered

    for (vector<double>::iterator it = data_outliers.begin(); it != data_outliers.end(); ++it)
        sample.push_back(*it);
...
void Model::writeIndividualTrackEfficienciesToCSVFile( string filepath )
{
    // setup file output and append information to the existing file if one exists
    ofstream output_stream;
    output_stream.open(filepath.c_str(), ios::out | ios::app);

    // Iterate over all searchers and get the number of targets found, the searcher id, and the distance and time spent searching
    Searcher* searchers = getSearchSpace()->getSearchers();
    for ( int i = 0; i < getSearchSpace()->getNumSearchers(); i++)
        output_stream << searchers[i].getID()
                      << ", "
                      << searchers[i].getTimeEfficiency()*(1/getSearchSpace()->getTimeResolution())
                      << ", "
                      << searchers[i].getDistEfficiency()
                      << ", "
                      << searchers[i].getDistanceTravelled()
                      << ", "
                      << searchers[i].getPath().size()*getSearchSpace()->getTimeResolution()
                      << ", "
                      << searchers[i].getNumTargetsFound()
                      << endl;
...
void Model::setSavePath(QString path)
{
    save_path = path.toStdString();
    saveSettings();
    cout << "Model:setSavePath(): save path set to " << save_path << endl;
}

string Model::getSavePath()
{
    return save_path;
}
