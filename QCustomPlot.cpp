...
/*!
  Creates a new QCPScatterStyle instance with shape set to \a shape and size to \a size. No pen orhttp://open.spotify.com/track/2RpcmzTSSpdRZRhcUISf3w
  brush is defined.
...
QCPStatisticalBox::QCPStatisticalBox(QCPAxis *keyAxis, QCPAxis *valueAxis) :
  ...
  mMean(0)
{
  setOutlierStyle(QCPScatterStyle(QCPScatterStyle::ssPlus, Qt::gray, 5));
  setMeanStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, Qt::gray, 5));
...
void QCPStatisticalBox::setMean(double value)
{
  mMean = value;
}
...
void QCPStatisticalBox::setMeanPen(const QPen &pen)
{
  mMeanPen = pen;
}
...
void QCPStatisticalBox::setMeanStyle(const QCPScatterStyle &style)
{
  mMeanStyle = style;
}
...
void QCPStatisticalBox::draw(QCPPainter *painter)
{
  ...
  if (QCP::isInvalidData(mKey, mMedian) ||
      ...
      QCP::isInvalidData(mMinimum, mMaximum) || QCP::isInvalidData(mMean))
...
//Draws the mean line inside the quartile box.

void QCPStatisticalBox::drawMean(QCPPainter *painter) const
{

    applyScattersAntialiasingHint(painter);
    mMeanStyle.applyTo(painter, mMeanPen);
    mMeanStyle.drawShape(painter, coordsToPixels(mKey, mMean));

      //applyDefaultAntialiasingHint(painter);
    //painter->setPen(mMeanPen);
    //painter->drawEllipse(coordsToPixels((mKey+mWidth)/2, mMean), mWidth*0.1, mWidth*0.1);

//QLineF meanLine;
//meanLine.setP1(coordsToPixels(mKey-mWidth*0.5, mMean));
//meanLine.setP2(coordsToPixels(mKey+mWidth*0.5, mMean));
//applyDefaultAntialiasingHint(painter);
//painter->setPen(mMeanPen);
//painter->drawLine(meanLine);
}
...
QCPRange QCPStatisticalBox::getValueRange(bool &foundRange, SignDomain inSignDomain) const
{
  ...
  values << mMaximum << mUpperQuartile << mMedian << mLowerQuartile << mMinimum << mMean;
  values << mOutliers;
