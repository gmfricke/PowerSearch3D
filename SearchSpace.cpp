...
float SearchSpace::getFromMaxwellPDF()
{
    /*
    /// <summary>
            ...
            Another approach:
            http://www.physics.unlv.edu/~pang/comp5/maxwell.c

            */
}

float SearchSpace::getFromHarrisPowerLawPDF()
{
    // Mathamatica
...
{
    float xmin = 6.5350e-04; // From observation
    float unif_var = rand()*1.0/RAND_MAX; // This should actually be ~0.9 from observed max
    float exponent = 2.15;//1.2; // From observation
    float scale = (exponent-1)/xmin;
    return scale*pow(unif_var/xmin, -exponent);
}

float SearchSpace::getFromLogNormalPDF()
{
    float normal_var = getFromNormalPDF( -2.5, 0.9329);
