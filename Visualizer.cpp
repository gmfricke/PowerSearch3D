...
void Visualizer::GUISetClusterRadius(const QString &textvalue)
{
    ...
    QStringList cluster_radii_str = textvalue.split(',');

    FloatVector cluster_radii;

    for ( int i = 0; i < cluster_radii_str.size(); i++ )
    {
        cluster_radii.push_back(cluster_radii_str[i].toFloat());
    }

    emit VisSetClusterRadius(cluster_radii);
}
